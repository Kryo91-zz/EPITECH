...
margarita::margarita(int fd, std::string size, double multiplier, std::chrono::milliseconds time)
{
    this->_bake = false;
    ...
    while (this->_bake == false) {
        ...
        if (this->work_time.count() >= (1000.0 * multiplier) && ing.margarita() == true) {
            tmp = "Margarita " + size + " is done ";
            write(fd, tmp.c_str(), strlen(tmp.c_str()));
            tmp.clear();
            b = std::chrono::system_clock::now();
            this->_bake = true;
        } else {
                if (ing.check_margarita() == false)
